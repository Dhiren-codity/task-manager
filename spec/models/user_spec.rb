...
RSpec.describe User do
  ...
  describe '#complete_task!' do
    ...
    context 'when task completion fails' do
  ...
  describe '#high_karma?' do
    it 'returns true if karma is above or equal to the threshold' do
      user.karma = User::HIGH_KARMA_THRESHOLD
      expect(user.high_karma?).to be true
    end

    it 'returns false if karma is below the threshold' do
      user.karma = User::HIGH_KARMA_THRESHOLD - 1
